...
if (!AudioWorkletGlobalScope.WAMProcessor) {
  class WAMProcessor extends AudioWorkletProcessor {
    ...
    constructor(options) {
      ...
      const WAM = options.mod
      this.WAM = WAM
      WAM.port = this.port

      // -- construction C function wrappers
      const wam_ctor = WAM.cwrap("createModule", "number", [])
      const wam_init = WAM.cwrap("wam_init", null, [
        "number",
        "number",
        "number",
        "string",
      ...
      this.inst = wam_ctor()
      const desc = wam_init(this.inst, this.bufsize, this.sr, "")
      ...
      const ibufs = this.numInputs > 0 ? WAM._malloc(this.numInputs) : 0
      const obufs = this.numOutputs > 0 ? WAM._malloc(this.numOutputs) : 0
      ...
      for (var i = 0; i < this.numOutputs; i++) {
        const numChannels = 2 // this.numOutChannels[i];
    ...
    onmsg(verb, prop, data) {
      if (data instanceof ArrayBuffer) {
        ...
        const { WAM } = this
        const buf = WAM._malloc(data.byteLength)
        for (let i = 0; i < len; i++) {
    ...
    onsysex(data) {
      const { WAM } = this
      const buf = WAM._malloc(data.length)
    ...
    process(inputs, outputs) {
      ...
      for (let i = 0; i < this.numInputs; i++) {
        const waain = inputs[i][0]
        const wamin = this.audiobufs[0][i]
      ...
      for (let i = 0; i < this.numOutputs; i++) {
        const numChannels = this.numOutChannels[i]
        for (let c = 0; c < numChannels; c++) {
          const waaout = outputs[i][c]
          const wamout = this.audiobufs[1][i * numChannels + c]
