...
},{"./lib/Patch":2,"./lib/parsing":3,"./lib/pd-rendering":4,"./lib/svg-rendering":5}],2:[function(require,module,exports){
/*
...
exports.render = function(patch, opts) {
  ...
  var svgContainer = d3.select('body').append('div')
    , svg = svgContainer.append('svg')
      //.attr('style', 'font-family:Tomo, monospace')
    ...
    .each(function(conn) {
      ...
      d3.select(this)
        .attr('x1', function(conn) {
          return ~~(sourceRenderer.getOutletX(conn.source.port) + opts.portletWidth/2)+.5
        })
        .attr('y1', function(conn) {
          return ~~(sourceRenderer.getOutletY(conn.source.port) + opts.portletHeight)+.5
        })
        .attr('x2', function(conn) {
          return ~~(sinkRenderer.getInletX(conn.sink.port) + opts.portletWidth/2)+.5
        })
        .attr('y2', function(conn) {
          return ~~(sinkRenderer.getInletY(conn.sink.port))+.5
        })
    })
  ...
  root.attr('transform', 'translate('
    + (~~(-topLeft.x + opts.padding)+.5) + ' '
    + (~~(-topLeft.y + opts.padding)+.5) + ')'
  )
