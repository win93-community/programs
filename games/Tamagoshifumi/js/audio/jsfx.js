...
jsfx.SoundsImmediate = function(library) {
  ...
  const player = {};
  player._audio = audio;
  map_object(library, (params, name) => {
...
jsfx.FloatBuffer = function(params) {
  const processor = new Processor(params, jsfx.DefaultModules);
  const block = createFloatArray(processor.getSamplesLeft());
  processor.generate(block);
  return block;
...
jsfx.Module.Frequency = {
  ...
  process($, block) {
    ...
    const min = +$.phaseSpeedMin;
    const max = +$.phaseSpeedMax;
    let slide = +$.phaseSlide;
    const deltaSlide = +$.phaseDeltaSlide;

    let { repeatTime, repeatLimit } = $;

    let arpTime = $.arpeggiatorTime;
    let arpLimit = $.arpeggiatorLimit;
...
jsfx.Module.Vibrato = {
  ...
  process($, block) {
    ...
    let depth = +$.vibratoDepth;
    let speed = +$.vibratoPhaseSpeed;
    const slide = +$.vibratoPhaseSpeedSlide;
    const depthSlide = +$.vibratoDepthSlide;
...
jsfx.Module.Phaser = {
  ...
  process($, block) {
    ...
    let offsetSlide = +$.phaserOffsetSlide;
...
jsfx.Preset = {
  ...
  Lucky() {
    const p = EmptyParams();
    map_object(p, (out, moduleName) => {
      const defs = jsfx.Module[moduleName].params;
      map_object(defs, (def, name) => {
        if (def.C) {
          const values = Object_keys(def.C);
          out[name] = values[(values.length * Math.random()) | 0];
        } else {
          out[name] = (Math.random() * (def.H - def.L) + def.L).toFixed(3);
        }
      });
    });
    p.Volume.Master = 0.4;
...
// STATEFUL
const __noiseLast = 0; // eslint-disable-line no-unused-vars
...
function CopyFToU8(into, floats) {
  ...
  for (let i = 0; i < floats.length; i++) {
    const v = +floats[i];
    let a = (v * 0x7fff) | 0;
    a = a < -0x8000 ? -0x8000 : a > 0x7fff ? 0x7fff : a;
    a += a < 0 ? 0x10000 : 0;
    into[k] = a & 0xff;
    k++;
    into[k] = a >> 8;
