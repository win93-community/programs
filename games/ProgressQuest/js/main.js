...
function Plural(s) {
  ...
  if (Ends(s, "ch") || Ends(s, "x") || Ends(s, "s") || Ends(s, "sh")) {
    return s + "es"
  }
...
function InterplotCinematic() {
  switch (Random(3)) {
    ...
    case 1:
      ...
      var s = Random(3)
      for (let i = 1; i <= Random(1 + game.act + 1); ++i) {
        s += 1 + Random(2)
        switch (s % 3) {
          case 0:
...
function MonsterTask(level) {
  ...
  return { description: result, level }
}
...
function ProperCase(s) {
  return Copy(s, 1, 1).toUpperCase() + Copy(s, 2, 10_000)
}
...
function Dequeue() {
  while (TaskDone()) {
    if (Split(game.task, 0) == "kill") {
      ...
      if (game.task == "sell") {
        let amt = GetI(Inventory, 1) * GetI(Traits, "Level")
    ...
    if (game.queue.length > 0) {
      const a = Split(game.queue[0], 0)
      const n = StrToInt(Split(game.queue[0], 1))
      let s = Split(game.queue[0], 2)
      if (a == "task" || a == "plot") {
        game.queue.shift()
        if (a == "plot") {
          CompleteAct()
      ...
      let nn = GetI(Traits, "Level")
      const t = MonsterTask(nn)
      const InventoryLabelAlsoGameStyleTag = 3
      nn = Math.floor(
...
function Put(list, key, value) {
  ...
  if (list === Inventory) {
    let cubits = 0
    $.each(game.Inventory.slice(1), (index, item) => {
...
function ProgressBar(id, tmpl) {
  this.id = id
  this.bar = document.querySelector(`#${id}`)
  this.tmpl = tmpl
  ...
  this.reposition = function (newpos) {
    ...
    // Update UI
    if (this.bar) {
      const p = this.Max() ? (100 * this.Position()) / this.Max() : 0
      this.bar.value = p / 100
      this.bar.dataset.label = game[this.id].hint
    }
  }
...
function ListBox(id, columns, fixedkeys) {
  this.id = id
  this.box = $("tbody#_, #_ tbody".replaceAll("_", id))
  ...
  this.AddUI = function (caption) {
    if (!this.box) return
    const tr = $(
      "<tr><td><input type=checkbox disabled><span>" +
        caption +
        "</span></td></tr>",
    )
  ...
  this.PutUI = function (key, value) {
    if (!this.box) return
    let item = this.rows().filter(function (index) {
      return Key(this) === key
    })
    if (item.length === 0) {
  ...
  this.load = function (game) {
    const that = this
    const dict = game[this.id]
    if (this.fixedkeys) {
      $.each(this.fixedkeys, (index, key) => {
...
function LPick(list, goal) {
  ...
  for (let i = 1; i <= 5; ++i) {
    const best = StrToInt(Split(result, 1))
    const s = Pick(list)
    const b1 = StrToInt(Split(s, 1))
...
function WinEquip() {
  const posn = Random(Equips.length())
  ...
  let name = LPick(stuff, GetI(Traits, "Level"))
  let qual = StrToInt(Split(name, 1))
  name = Split(name, 0)
  let plus = GetI(Traits, "Level") - qual
...
function Get(list, key) {
  ...
  for (let i = 0; i < game[list.id].length; ++i) {
    if (game[list.id][i][0] === key) {
...
function RoughTime(s) {
  ...
  if (s < 60 * 120) return s.div(60) + " minutes"
  if (s < 60 * 60 * 48) return s.div(3600) + " hours"
  if (s < 60 * 60 * 24 * 60) return s.div(3600 * 24) + " days"
  if (s < 60 * 60 * 24 * 30 * 24) return s.div(3600 * 24 * 30) + " months"
  return s.div(3600 * 24 * 30 * 12) + " years"
}
...
function Timer1Timer() {
  if (TaskBar.done()) {
    ...
    if (game.kill == "Loading....") {
      TaskBar.reset(0)
    } // Not sure if this is still the ticket

    // gain XP / level up
    const gain = Pos("kill|", game.task) == 1
    if (gain) {
      if (ExpBar.done()) {
        LevelUp()
      } else {
    ...
    let elapsed = timeGetTime() - clock.lasttick
...
function FormCreate() {
  ...
  Plots.load = function (sheet) {
    for (let i = Max(0, game.act - 99); i <= game.act; ++i) {
  ...
  const name = unescape(window.location.href.split("#")[1])
...
function HotOrNot() {
  ...
  if (Spells.length()) {
    const flat = 1 // Flattening constant
    var best = 0
    var i
...
function LoadGame(sheet) {
  ...
  // if (document) {
  //   let title = "Progress Quest - " + GameSaveName()
  //   $("#title").text(title)
  ...
  randseed(game.seed)
  $.each(AllBars.concat(AllLists), (i, e) => {
    e.load(game)
  })
  if (Kill) {
    Kill.text(game.kill)
  ...
  function patch(from, to) {
    function count(spell) {
      const t = game.Spells.filter((a) => a[0] == spell)
...
function FormKeyDown(e) {
  ...
  if (e.key === "d") {
    alert("Your character's genome is " + ToDna(String(game.dna)))
  }
  ...
  if (game.online) {
    ...
    if (e.key === "m") {
      const mot = InputBox("Declare your motto!", game.motto)
      if (mot !== null) {
        game.motto = mot
        Brag("m", true)
      }
  ...
  if (e.key === "w") {
    ...
    SaveGame(() => {
      const ext = window.open(
        window.location.href,
        "Progress Quest",
        `resizable,width=${$("#main")[0].offsetWidth},height=${$("#main")[0].offsetHeight},popup,location=0`,
      )
      console.log(ext)
      if (ext && !ext.closed && ext.closed !== undefined) {
        // popup was apparently not blocked
        window.location.href = "index.html" // this window can go back to the roster
      }
    })
  }
...
function Brag(trigger, andSeeIt) {
  ...
  if (game.online) {
    ...
    for (trait in game.Traits) {
      url += `&${LowerCase(trait.slice(0, 1))}=${UrlEncode(game.Traits[trait])}`
    }
...
function Guildify(guild) {
  ...
  for (trait in game.Traits) {
    url +=
      "&" + LowerCase(trait.slice(0, 1)) + "=" + UrlEncode(game.Traits[trait])
  }
