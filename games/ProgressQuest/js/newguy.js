...
function RollEm() {
  ...
  if (document) {
    ...
    $("#Unroll").attr("disabled", seedHistory.length === 0)
  }
}
...
function fill(e, a, n) {
  const def = Random(a.length)
  for (let i = 0; i < a.length; ++i) {
    const v = a[i].split("|")[0]
    const check = def == i ? " checked " : " "
    if (def == i) traits[n] = v
    if (document) {
      $(
        `<div><input type=radio id='${v}' name="${n}" value="${v}" ${check}><label for='${v}'>${v}</label></div>`,
      ).appendTo(e)
    }
  }
}
...
function sold() {
  ...
  //       newguy.online.passkey = Number.parseInt(body.split("|")[1])
  //       charIsBorn(newguy)
  //     } else {
